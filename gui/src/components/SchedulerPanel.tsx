  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-10 animate-in fade-in duration-500">
      ...
      <section className="bg-surface-light border-2 border-surface-lighter p-8 rounded-3xl shadow-xl space-y-8 relative overflow-hidden group">
        ...
        <div className="flex flex-wrap items-end gap-6 relative z-10 pt-4 border-t border-surface-lighter/50">
                ...
                <div className="flex bg-surface p-1.5 rounded-2xl border border-surface-lighter gap-1">
                    {(['interval', 'daily', 'once'] as const).map(t => (
                        <button 
                            ...
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${newTask.type === t ? 'bg-primary text-neutral-900 shadow-lg' : 'text-text-dim hover:text-text-main hover:bg-surface-light'}`}
                        >
                            {t}
                        </button>
                    ))}
            ...
            <button 
                ...
                className="bg-primary text-neutral-900 h-[50px] px-8 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-30 ml-auto"
            >
                Authorize Schedule
            </button>
        </div>
      ...
      <div className="space-y-6">
                ...
                <div key={task.id} className="bg-surface border border-surface-lighter p-6 rounded-3xl flex items-center gap-8 hover:border-primary/50 transition-colors group relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 border-l border-surface-lighter pl-6">
                         <button 
                            ...
                            className="p-3 text-text-dim hover:text-red-400 transition-colors hover:bg-red-500/5 rounded-2xl"
                         >
                            <Trash2 size={20} />
                         </button>
                    </div>
       ...
       <footer className="pt-10 border-t border-surface-lighter flex items-center justify-between text-[10px] font-black text-text-dim/40 uppercase tracking-[0.2em]">
        <div className="flex items-center gap-4">
             <Check size={14} className="text-primary" />
             Persistence Matrix Synchronized
        </div>
        <div className="flex items-center gap-2">
            CRON STATUS: <span className="text-primary">RUNNING</span>
        </div>
      </footer>
    </div>
  )
