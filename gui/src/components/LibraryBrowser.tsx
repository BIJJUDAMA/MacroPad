...
export function LibraryBrowser() {
    ...
    const { macros, loading, error, refresh, addMacro, removeMacro } = useMacroList()
    const [search, setSearch] = useState("")
    const [tagFilter, setTagFilter] = useState<string | null>(null)

    const allTags = Array.from(
        new Set(macros.flatMap(m => m.tags))
    ).sort()
    ...
    async function handleBrowse() {
        try {
            ...
            if (path) {
                addMacro(path)
                console.log("added macro, current macros:", macros)
            }
        } catch (e) {
        ...
        <div style={styles.root}>
            <div style={styles.toolbar}>
                ...
                <div style={styles.right}>
                    <button style={styles.btnAdd} onClick={handleBrowse}>
                        + add macro
                    </button>
                    <button style={styles.btnRefresh} onClick={refresh}>
                        refresh
                    </button>
                </div>
            </div>
            ...
            <div style={styles.filters}>
                ...
                <div style={styles.tagRow}>
                    <span
                        style={{
                            ...styles.tagFilter,
                            background: tagFilter === null ? "#89b4fa" : "#313244",
                            color: tagFilter === null ? "#1e1e2e" : "#cdd6f4",
                        }}
                    ...
                    {allTags.map(tag => (
                        <span
                            ...
                            style={{
                                ...styles.tagFilter,
                                background: tagFilter === tag ? "#89b4fa" : "#313244",
                                color: tagFilter === tag ? "#1e1e2e" : "#cdd6f4",
                            }}
