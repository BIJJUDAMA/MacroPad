...
function blocksToSource(blocks: BlockStatement[]): string {
    function render(block: BlockStatement, indent: number): string {
        ...
        switch (block.type) {
            case "let":
                return `${pad}let ${block.args.name} = "${block.args.value}"`
            case "run":
                return `${pad}run "${block.args.path}"`
            case "run_async":
                return `${pad}run_async "${block.args.path}"`
            case "delay":
                return `${pad}delay ${block.args.ms}`
            case "wait_for":
                return `${pad}wait_for ${block.args.condition} timeout=${block.args.timeout}`
            case "loop":
                return [
                    `${pad}loop(${block.args.count}) {`,
                    ...block.children.map(c => render(c, indent + 1)),
                    `${pad}}`,
                ].join("\n")
            case "loop_while":
                return [
                    `${pad}loop_while ${block.args.condition} max=${block.args.max} {`,
                    ...block.children.map(c => render(c, indent + 1)),
                    `${pad}}`,
                ].join("\n")
            case "if":
                return [
                    `${pad}if ${block.args.condition} {`,
                    ...block.children.map(c => render(c, indent + 1)),
                    `${pad}}`,
                ].join("\n")
            case "elif":
                return [
                    `${pad}elif ${block.args.condition} {`,
                    ...block.children.map(c => render(c, indent + 1)),
                    `${pad}}`,
                ].join("\n")
            case "else":
...
export function ScriptEditor() {
    ...
    function handleViewToggle(next: ScriptView) {
        if (next === "blocks" && view === "code") {
            setBlocks([])
        }
        if (next === "code" && view === "blocks") {
            setSource(blocksToSource(blocks))
        }
        setView(next)
    }
            ...
            <div style={styles.toolbar}>
                ...
                <div style={styles.right}>
                    <div style={styles.toggle}>
                        <button
                            style={{ ...styles.toggleBtn, ...(view === "code" ? styles.toggleActive : {}) }}
                            onClick={() => handleViewToggle("code")}
                        >
                            code
                        </button>
                        <button
                            style={{ ...styles.toggleBtn, ...(view === "blocks" ? styles.toggleActive : {}) }}
            ...
            {!path ? (
                <div style={styles.empty}>
                    ...
                    <div style={styles.emptyActions}>
                        ...
                        <button style={styles.emptyBtn} onClick={handleOpen}>
                            open existing
                        </button>
                    </div>
                </div>
