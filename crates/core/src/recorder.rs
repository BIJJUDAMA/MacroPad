...
impl Recorder {
    ...
    pub fn start_with_options(opts: RecordingOptions) -> Result<Self, RecorderError> {
        ...
        thread::spawn(move || {
            ...
            println!(
                ">>DEBUG: macropad_core: Low-level capture thread started. Hooking OS events."
            );
                ...
                move |rdev_event: rdev::Event| {
                    ...
                    match &rdev_event.event_type {
                        ...
                        RdevEventType::MouseMove { x, y } => {
                            ...
                            if dist < opts.min_move_distance_px || dt < opts.min_move_interval_ms {
                                return;
                            }
...
pub fn consolidate_mouse_segments(events: &[Event]) -> Vec<Event> {
    ...
    if !move_buffer.is_empty() {
        ...
        result.push(Event {
            ...
            duration_ms: Some(0),
            waypoints: if move_buffer.len() > 2 {
                Some(move_buffer)
            } else {
                None
            },
